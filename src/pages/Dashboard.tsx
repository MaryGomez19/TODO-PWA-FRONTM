...
function normalizeTask(x: any): Task {
  ...
  return {
    _id: id ? String(id) : crypto.randomUUID(), // ⬅️ garantiza siempre un ID válido
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Un ÚNICO listener online/offline
    const on = async () => {
      setOnline(true);
      await syncNow();        // candado en sync.ts evita paralelos
      await loadFromServer(); // refresca desde server
    };
  ...
  async function addTask(e: React.FormEvent) {
    ...
    const localTask = normalizeTask({
      ...
      imagePublicId, // <- guarda public_id para futuras referencias
  ...
  async function saveEdit(taskId: string) {
    ...
    if (editingImage && navigator.onLine) {
      const formData = new FormData();
    ...
    const patched = { ...before, title: newTitle, description: newDesc, image: imageUrl } as Task;
    ...
    if (!navigator.onLine) {
      await queue({
        ...
        data: { title: newTitle, description: newDesc, image: imageUrl },
        ts: Date.now(),
      } as OutboxOp);
      return;
    }
    ...
    <div className="wrap">
      <header className="topbar">
        ...
        <div className="stats">
          ...
          <span className="badge" style={{ marginLeft: 8, background: online ? "#1feb33" : "#b40909" }}>
                ...
                <div className="content">
                  {editingId === t._id ? (
                    <>
                      ...
                      />
                      {editingImage && (
                        <img src={URL.createObjectURL(editingImage)} alt="preview" style={{ width: 80, borderRadius: 6 }} />
                      )}
                    </>
                  ) : (
                    <>
                      ...
                      {(t.pending || isLocalId(t._id)) && (
                        <span
                          ...
                          style={{ background: "#b486097c", width: "fit-content" }}
                        >
                          Falta sincronizar
                        </span>
                      )}
